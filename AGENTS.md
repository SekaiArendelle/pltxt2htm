...
- **Design context data structures, not one type per `NodeKind`:**
  - The frame/context types are **data-structure-driven**, not **type-driven**. A single context type serves *every*
    `NodeKind` that needs the same data layout; there is deliberately no one-to-one mapping between a `NodeKind` and a
    context class, enumerator, or union member.
  - Before adding a new context type, enumerator, or union member, check whether an existing data structure already
    carries the required fields. If it does, reuse it by adding the `NodeKind` to the existing `union`/`switch`
    cases and widening the relevant accessor assertions, instead of introducing a duplicate.
  - Name context types, enumerators, and union members after the **data they carry** (for example
    `...WithMarginsInfo`, `...WithBackgroundColorInfo`), never after a tag or node kind that merely happens to use them.
    A name like `...WithHtmlMarkInfo` or `...WithUnityMarginTagInfo` is a smell: it invites the false assumption that
    one `NodeKind` maps to one class.
  - When two kinds share a structure, make the shared case explicit in exhaustive tagged-union switches (for example
    `case html_mark: case unity_mark:` bound to one accessor result) and keep the switch exhaustive so `-Werror=switch`
    still flags newly added enumerators.
