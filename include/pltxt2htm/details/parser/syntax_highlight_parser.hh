...
template<::pltxt2htm::Contracts ndebug, SyntaxLanguage language>
    requires (language == SyntaxLanguage::c || language == SyntaxLanguage::cpp)
[[nodiscard]]
constexpr auto parse_c_family_code_syntax(::fast_io::u8string_view const content) noexcept -> ::pltxt2htm::Ast<ndebug> {
    ...
    while (lookahead_ast.empty() == false || remaining.empty() == false) {
        ...
        if (::pltxt2htm::details::syntax_is_identifier_start(chr)) {
            ...
            bool is_keyword{};
            if constexpr (language == SyntaxLanguage::c) {
                is_keyword =
...
constexpr auto parse_code_fence_syntax(::fast_io::u8string_view const content, SyntaxLanguage const language) noexcept
    ...
    case SyntaxLanguage::c:
        return ::pltxt2htm::details::parse_c_family_code_syntax<ndebug, SyntaxLanguage::c>(content);
    case SyntaxLanguage::cpp:
        return ::pltxt2htm::details::parse_c_family_code_syntax<ndebug, SyntaxLanguage::cpp>(content);
