...
enum class SimplePltextParseMode : unsigned {
    ...
    semantic, ///< Decode HTML character references but preserve backslashes.
};
...
constexpr auto parse_simple_pltext_node(::pltxt2htm::container::U8StringView pltext,
    ...
    if (auto const opt_space_size = ::pltxt2htm::details::try_parse_space<ndebug>(pltext); opt_space_size.has_value()) {
        ...
        return {.advance_count = space_size, .ascii = chr == u8' ' ? chr : char8_t{}};
    }
    if (chr == u8'&') {
...
constexpr auto simply_parse_pltext(::pltxt2htm::container::U8StringView pltext) noexcept
    ...
    while (current_index != pltext.size()) {
        ...
        current_index +=
            ::pltxt2htm::details::parse_simple_pltext_node<ndebug, SimplePltextParseMode::semantic>(remaining, ast)
                .advance_count;
    }
