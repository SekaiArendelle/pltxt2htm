...
constexpr auto regex_match(const char* str, const char* pattern) -> bool {
  ...
  const auto atom_size = *pattern == '\\' && pattern[1] != '\0' ? 2 : 1;
  const auto atom_matches =
      *str != '\0' && ((atom_size == 1 && *pattern == '.') || pattern[atom_size - 1] == *str);

  if (pattern[atom_size] == '*') {
    return regex_match(str, pattern + atom_size + 1) || (atom_matches && regex_match(str + 1, pattern));
  }

  return atom_matches && regex_match(str + 1, pattern + atom_size);
}
}  // namespace utility
