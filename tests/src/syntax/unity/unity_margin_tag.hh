...
TEST_SUITE("unity_margin_tag") {
    ...
    TEST_CASE("web backend renders <margin...> as a block-level div wit...") {
    ...
    TEST_CASE("<margin-right=1em>text</margin>") {
    ...
    TEST_CASE("<margin=2em>text</margin>") {
    ...
    TEST_CASE("a newline inside a margin block still renders as <br>") {
    ...
    TEST_CASE("text after the closing </margin> stays on its own conten...") {
    ...
    TEST_CASE("mid-line <margin> sequences are literal text (block-leve...") {
    ...
    TEST_CASE("a non-numeric value renders as literal text") {
    ...
    TEST_CASE("an empty margin block is kept; not erased by the optimiz...") {
    ...
    TEST_CASE("an unclosed margin block still parses") {
    ...
    TEST_CASE("px is the default unit and is emitted without a suffix") {
    ...
    TEST_CASE("zero is a valid margin value") {
    ...
    TEST_CASE("tag name matching is case-insensitive (TMP accepts <MARG...") {
    ...
    TEST_CASE("text after the closing </margin> starts on a new line in...") {
    ...
    TEST_CASE("two adjacent block-level margin tags are separated by a...") {
    ...
    TEST_CASE("text before and after a block-level margin") {
    ...
    TEST_CASE("an empty margin between two lines keeps its block bounda...") {
    ...
    TEST_CASE("the attribute form <margin left=V right=W> round-trips t...") {
    ...
    TEST_CASE("left and right margins can differ") {
    ...
    TEST_CASE("the attribute form may specify only one side") {
        auto html = ::pltxt2htm_test::pltxt2plunity_introduction(u8"<margin left=2em>text</margin>");
        auto const& answer = u8"<margin left=2em>text</margin>\n";
        CHECK(html == answer);
    }
    ...
    TEST_CASE("attribute names are case-insensitive (TMP uppercases the...") {
        auto html = ::pltxt2htm_test::pltxt2plunity_introduction(u8"<margin LEFT=2em RIGHT=3em>text</margin>");
        auto const& answer = u8"<margin left=2em right=3em>text</margin>\n";
        CHECK(html == answer);
    }
    ...
    TEST_CASE("<margin=V> and <margin left=V right=V> are equivalent") {
    ...
    TEST_CASE("a repeated attribute is malformed and renders as literal...") {
    ...
    TEST_CASE("a newline inside an attribute-form margin block still re...") {
    ...
    TEST_CASE("mid-line attribute-form <margin> sequences are literal t...") {
        auto html = ::pltxt2htm_test::pltxt4unittest(u8"t<margin left=2em>x</margin>t");
        auto const& answer = u8"t&lt;margin&nbsp;left=2em&gt;x&lt;/margin&gt;t";
        CHECK(html == answer);
    }
    ...
    TEST_CASE("a malformed attribute value renders as literal text") {
    ...
    TEST_CASE("an unknown attribute name renders as literal text") {
    ...
    TEST_CASE("a missing value renders as literal text") {
    ...
    TEST_CASE("<margin-left=2em><margin-right=2em>x</margin></margin>") {
        auto html =
